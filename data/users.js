...
async function removeUser(id) {
    ...
    if (deletionInfo.deletedCount === 0) throw "Could not delete user " + id.toString();
...
async function updateUser(id, updatedUser) {
    if (!id || typeof id !== "object") throw "Invalid ID parameter";
    if (!updatedUser || typeof updatedUser !== "object") throw "Invalid updatedUser parameter";

    const userCollection = await users();
    const updateUserData = {};

    if (updatedUser.firstName)
        updateUserData.firstName = updatedUser.firstName;
    if (updatedUser.lastName)
        updateUserData.lastName = updatedUser.lastName;
    if (updatedUser.email)
        updateUserData.email = updatedUser.email;
    if (updatedUser.age)
        updateUserData.age = updatedUser.age;
    if (updatedUser.password)
        updateUserData.password = updatedUser.password;
    if (updatedUser.cash)
        updateUserData.cash = updatedUser.cash;
